...
class MotoGPRealDataSimulator:
    ...
    def analyze_race_results(self, race_df: pd.DataFrame, qualifying_df: pd.DataFrame) -> Dict:
        ...
        running_results.sort(key=lambda x: x["final_position"])
        ...
        stats = {
            "avg_lap_time": valid_laps["lap_time"].mean() if not valid_laps.empty else None,
            "std_lap_time": valid_laps["lap_time"].std() if not valid_laps.empty else None,
            "dnf_count": len(dnf_results),
            "dnf_rate": len(dnf_results) / len(final_results) if final_results else 0
        }
    ...
    def run_complete_simulation(self, circuit_name: str, weather_condition: str = "dry") -> Dict:
        ...
        for _, row in qualifying_results.head(10).iterrows():
            print(f"{row['position']:2d}. {row['name']:<20} - {row['lap_time']:.3f}s")
        ...
        for i, result in enumerate(race_analysis["final_classification"]):
            if result["status"] == "Running":
                print(f"{result['final_position']:2d}. {result['name']:<20} - {result['points']} pts "
                     f"(Q: {result['qualifying_position']})")
            else:
                print(f"DNF. {result['name']:<20} - {result['status']} "
                     f"(Q: {result['qualifying_position']}, Tours: {result['laps_completed']})")
        ...
        with open(results_file, 'w', encoding='utf-8') as f:
            ...
            race_analysis["race_data"] = race_analysis["race_data"].to_dict('records')
            json.dump(race_analysis, f, ensure_ascii=False, indent=2)
